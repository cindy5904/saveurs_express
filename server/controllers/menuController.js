...
const menuController = {
    ...
    createMenu: async function (req, res) {
        try {
            const { nom, prix, description, image, userId, restaurantId } = req.body;
            ...
            await Menu.create({ nom, prix, description, image, PersonneId: userId, RestaurantId: restaurantId });
    ...
    getMenusByRestaurant: async function (req, res) {
        try {
            const { restaurantId } = req.params;
            const menu = await Menu.findAll({ where: { RestaurantId: restaurantId } });
            if (!menu) {
                return res.status(404).json({ message: "Menus non trouvés" });
            }
    ...
    updateMenu: async function (req, res) {
        try {
            ...
            await menu.update({ nom, prix, description, image });
